...
ggplot(
  ...
  geom_line(aes(group = isotope, color = isotope)) +
  geom_point(aes(group = isotope, color = isotope, shape = isotope)) +
  scale_y_log10(breaks = base_breaks(),
                labels = prettyNum) +
  scale_x_log10() +
  labs(x = "Concentration (ppb)", y = "Measured concentration, As or Se (ppb)") +
  scale_color_viridis_d(name = "Analyte", 
                        labels = parse_format()) +
  scale_shape_manual(name = "Analyte", 
                        labels = parse_format(), values = c(0, 15, 1, 16, 2, 17, 5)) +
  theme(axis.text.x = element_text(angle = 60,hjust = 1)) +
  facet_grid(mode ~ element, labeller = as_labeller(label_parsed))
...
ggplot(
  ...
  labs(x = "Concentration REEs (ppb)", y = "Measured concentration, As or Se (ppb)") +
  scale_color_viridis_d(name = "Actual \nConcentration \nAs or Se \n(ppb)") +
  theme(axis.text.x = element_text(angle = 60,hjust = 1)) +
  facet_grid(isotope ~ mode, labeller = as_labeller(label_parsed))
