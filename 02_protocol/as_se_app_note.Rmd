...
srs_mpv <- read.xlsx("./01_input/srs_data.xlsx", 
                 sheetIndex = 1, 
...
srs_fsigma <- read.xlsx("./01_input/srs_data.xlsx", 
                 sheetIndex = 1, 
                 rowIndex = c(11, seq(14, 52, by = 2))) %>%
  select(2, 5, 36, 15, 16, 17, 19, 20, 29, 38) %>%
  mutate(TRACE = srs_mpv$TRACE)

# Pivot and merge (or bind)

srs_mpv_longer <- pivot_longer(srs_mpv, 
                               2:10, 
                               names_to = "element", 
                               values_to = "mpv")

srs_fsigma_longer <- pivot_longer(srs_fsigma, 
                                  2:10, 
                                  names_to = "element", 
                                  values_to = "fsigma")

srs_all <- merge(srs_mpv_longer, srs_fsigma_longer) %>%
  mutate(sample = TRACE, .before = element, .keep = "unused") %>%
  mutate(mpv = gsub("ID", NA, mpv))

srs_appnote <- filter(srs_all, sample %in% c("T239", "T243", "T249", "T223", "T227"))

levels(as.factor(srs_appnote$sample))

ggplot(conc_srs %>%
         filter(conc < 1000), aes(x = mode, y = conc))+ #, color = mode, shape = mode
  geom_point(position = position_dodge(width = 0.5)) +
  scale_shape_manual(name = "mode", values = c(0, 15, 1, 16, 2, 17, 5)) +
  scale_y_log10() + 
  geom_hline(yintercept = 2, color = "red", linetype = 2) +
  facet_grid(samp_id ~ isotope, scales = "free_y")

```


From Verplank et al. 2001, As: 0.005 and 0.002 mg/L in PPREE1 and SCREE1, respectively, and no data for Se as determined by GFAA

```{r}
ppree_scree_ree <- 
  data.frame(element = c("As", "Se", "Nd", "Sm", "Eu", "Gd", "Dy", "Ho", "Er"),
             ppree1_mpv = c(1.5, 3.4, 92.3, 20.3, 5.95, 23.8, 22.0, 4.43, 11.9),
             ppre1_mad = c(0.1, NA, 5.7, 1.5, 0.48, 1.7, 0.7, 0.09, 0.4),
